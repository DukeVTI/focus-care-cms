...
const youngPersonSchema = z.object({
  ...
  residingLocalAuthority: z.string().min(1, "Residing local authority is required"),
  previousPlacementCategory: z.string().optional(),
  reasonForPlacement: z.string().min(1, "Reason for placement is required"),
  reasonForPlacementNotes: z.string().optional(),
  immigrationLegalStatus: z.string().min(1, "Legal status (immigration) is required"),
  careLegalStatus: z.string().min(1, "Children's legal status is required"),
  timeLookedAfter: z.string().optional(),
  ...
  healthSupport: z.string().optional(),
  ...
  offendingHistory: z.string().optional(),
  offendingDetails: z.object({
...
const steps = [
  ...
  { 
    ...
    fields: ["placementType", "placementStartDate", "placementAddress", "placementRoadName", "placementPostcode", "legalStatus", "placingAuthority", "residingLocalAuthority"]
  },
  { 
    id: 3, 
    title: "Key Contacts", 
...
export const NewYoungPersonWizard = ({ editMode = false, existingData }: WizardProps) => {
  ...
  const form = useForm<YoungPersonFormValues>({
    ...
    defaultValues: existingData ? {
      ...
      idType: existingData.id_type || "",
      idValue: existingData.id_value || "",
      ...
      previousPlacementCategory: existingData.previous_placement || "",
      reasonForPlacement: existingData.reason_for_placement || "",
      reasonForPlacementNotes: existingData.reason_for_placement_notes || "",
      immigrationLegalStatus: existingData.legal_status || "",
      careLegalStatus: existingData.care_legal_status || "",
      timeLookedAfter: existingData.time_looked_after || "",
      ...
      socialMediaAccounts: existingData.social_media_accounts || [],
      structuredIds: existingData.structured_ids || [],
      associatedAreas: existingData.associated_areas || [],
      associatedAreasNotes: existingData.associated_areas_notes || "",
      exploitationCategories: existingData.exploitation_categories || [],
      exploitationNotes: existingData.exploitation_notes || "",
      yotInvolved: existingData.yot_involved || false,
      yotWorkerName: existingData.yot_worker_name || "",
      yotWorkerPhone: existingData.yot_worker_phone || "",
      yotWorkerEmail: existingData.yot_worker_email || "",
      probationOrder: existingData.probation_order || "",
      probationEndDate: existingData.probation_end_date || "",
      documents: [],
      ...
      placementPostcode: "",
      placingAuthority: "",
      residingLocalAuthority: "",
      previousPlacementCategory: "",
      reasonForPlacement: "",
      reasonForPlacementNotes: "",
      immigrationLegalStatus: "",
      careLegalStatus: "",
      timeLookedAfter: "",
      legalStatus: "",
      ...
      structuredIds: [],
      associatedAreas: [],
      associatedAreasNotes: "",
      exploitationCategories: [],
      exploitationNotes: "",
      yotInvolved: false,
      yotWorkerName: "",
      yotWorkerPhone: "",
  ...
  const saveYoungPerson = async (values: YoungPersonFormValues, isDraft: boolean) => {
    ...
    const youngPersonData = {
      ...
      previous_placement: values.previousPlacementCategory || null,
      reason_for_placement: values.reasonForPlacement || null,
      reason_for_placement_notes: values.reasonForPlacementNotes || null,
      immigration_legal_status: values.immigrationLegalStatus || null,
      care_legal_status: values.careLegalStatus || null,
      time_looked_after: values.timeLookedAfter || null,
      ...
      exploitation_categories: values.exploitationCategories || [],
      exploitation_notes: values.exploitationNotes || null,
      yot_involved: values.yotInvolved,
      yot_worker_name: values.yotWorkerName || null,
      yot_worker_phone: values.yotWorkerPhone || null,
      yot_worker_email: values.yotWorkerEmail || null,
      probation_order: values.probationOrder || null,
      probation_end_date: values.probationEndDate || null,
      social_media_accounts: (values.socialMediaAccounts || []).filter((a: any) => a.platform?.trim() && a.handle?.trim()),
      structured_ids: (values.structuredIds || []).filter((i: any) => i.type?.trim() && i.value?.trim()),
      associated_areas: values.associatedAreas || [],
      associated_areas_notes: values.associatedAreasNotes || null,
